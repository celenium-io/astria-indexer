...
import (
	...
	"github.com/aopoltorzhicky/astria/cmd/api/cache"
	"github.com/aopoltorzhicky/astria/cmd/api/handler"
	"github.com/aopoltorzhicky/astria/cmd/api/handler/websocket"
...
func initHandlers(ctx context.Context, e *echo.Echo, cfg Config, db postgres.Storage) {
	...
	addressHandlers := handler.NewAddressHandler(db.Address, db.Tx, db.Action, db.Rollup, db.State, cfg.Indexer.Name)
	...
	rollupsHandler := handler.NewRollupHandler(db.Rollup, db.Action, db.State, cfg.Indexer.Name)
