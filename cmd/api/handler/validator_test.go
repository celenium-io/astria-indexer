...
func (s *ValidatorTestSuite) TestGet() {
	...
	s.Require().EqualValues("name", validator.Name)
...
func (s *ValidatorTestSuite) TestList() {
	...
	s.Require().EqualValues("name", validators[0].Name)
	s.Require().EqualValues("012345", validators[0].ConsAddress)
}

func (s *ValidatorTestSuite) TestBlocks() {
	req := httptest.NewRequest(http.MethodGet, "/", nil)
	rec := httptest.NewRecorder()
	c := s.echo.NewContext(req, rec)
	c.SetPath("/validators/:id/blocks")
	c.SetParamNames("id")
	c.SetParamValues("1")

	s.blocks.EXPECT().
		ByProposer(gomock.Any(), uint64(1), 10, 0, sdk.SortOrderDesc).
